...
如果写入时，等待接收队列非空(recvq),那么直接将数据给到等待的协程，不用经过缓冲区
...
#### GC时机
- 主动触发：调用 runtime.GC
- 被动触发：1. 超过两分钟没有GC的时候就触发一次 2. 堆内存分配到达阈值
#### V1.3 标记清除法
从根节点开始扫描，检测出可达与不可达的对象， 将不可达的对象清除掉。但是容易造成heap碎片并且GC时需要暂停程序（STW）。
#### V1.5 三色标记法
从根节点开始扫描，将扫描到的全部节点置为灰色，放进等待队列。
再依次扫描等待队列，将扫描到的灰色节点置为黑色，移出等待队列，再将灰色节点引用到的节点也置为灰色，放入等待队列。
重复上一步骤直到等待队列中再无节点，之后就可将所有白色节点清除。
缺点：如果在GC过程中用户手动的修改了三色的关系，则有可能发生GC错误；

为了解决这一问题，提出了两大范式：强三色不变式和弱三色不变式
- 强三色不变式：黑色节点不允许直接引用白色节点
- 弱三色不变式：黑色节点在引用白色节点时，白色节点的可达链路上必须存在灰色节点
...
### 6. context 上下文 是什么 / 有什么用
1. 是go语言标准库里的一个包， 用来请求的调用链上传递控制信息 （截止时间，取消信号，键值对）
2. 在项目中一般有以下两个使用场景：
   * 控制I/O操作 ： 
     1. 防止雪崩 —— 上层已经返回，下层还在运行，导致遇到流量高峰时将集群拖垮
     2. 防止资源泄露 —— 一个请求有时会并发调用不同的业务，只要其中一个返回错误，就需要关闭所有分支，做到同时退出
   * 携带令牌，并将令牌的超时时间映射为context超时

### 7. 并行赋值 / 语法糖 / 计算顺序
在并行赋值中，go会先完整计算右边的值，在统一赋值给左边
* 新值不会干扰计算
* 交换/滚动 不需要临时变量
```go
f0, f1 = f1, f1+f0
